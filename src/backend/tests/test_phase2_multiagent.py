...
class TestAgentRegistry:
    def test_tool_map_builds(self):
        from hagent.agent.orchestration import get_tool_map
    ...
    def test_resolve_tools(self):
        from hagent.agent.orchestration import resolve_tools
...
class TestCoordinatorRouting:
    ...
    def test_routing_instruction_dynamic(self):
        from hagent.agent.orchestration.coordinator import _build_routing_instruction
    ...
    def test_valid_agents_from_registry(self):
        from hagent.agent.orchestration.coordinator import _get_valid_agents
...
class TestDynamicGraph:
    ...
    def test_coordinator_route_end(self):
        ...
        from hagent.agent.orchestration.graph import coordinator_route
    ...
    def test_subagent_route_tools(self):
        ...
        from hagent.agent.orchestration.graph import subagent_route
    ...
    def test_after_sub_tools_routes_back(self):
        from hagent.agent.orchestration.graph import after_sub_tools
...
class TestContextExtraction:
    def test_data_analyst_extract(self):
        ...
        tool_msg = ToolMessage(
            content=json.dumps(
                {"id": "ds1", "name": "iris.csv", "n_rows": 150, "n_cols": 5}
            ),
    ...
    def test_evaluator_extract(self):
        ...
        msgs = [
            ToolMessage(
                content=json.dumps(
                    {"id": "j1", "best_model": "RF", "best_score": 0.92}
                ),
                name="get_job_info",
                tool_call_id="c1",
            ),
            ToolMessage(
                content=json.dumps(
                    {"id": "j2", "best_model": "XGB", "best_score": 0.95}
                ),
                name="get_job_info",
                tool_call_id="c2",
            ),
