"""
Tests: plan_executor loop + reviser (no live HAutoML / optional langchain).
"""
...
from hagent.agent.execution.reviser import reviser_node, reviser_route, _patch_plan_for_error
from hagent.agent.execution.tool_runner import set_tool_invoker
from hagent.agent.graph import coordinator_route, _should_run_plan_executor
...
def _reset_invoker(monkeypatch):
    ...
    # Campaign / hierarchy would steal train goals — keep executor tests focused
    monkeypatch.setattr(
...
class TestCoordinatorPlanRoute:
    ...
    def test_should_not_run_for_respond(self):
        ...
        # still has steps — gtype respond should skip
...
class TestPlanExecutor:
    ...
    def test_execute_until_done(self):
        ...
        st2 = {**st, **out1}
        # messages is list reducer style — replace for unit test
        st2["messages"] = []
    ...
    def test_tool_error_triggers_revise(self):
        ...
        out = run(plan_executor_node(st))
        assert out["plan_status"] == "need_revise"
        assert "backend down" in (out.get("last_step_error") or "")
...
class TestReviser:
    ...
    def test_reviser_budget(self):
        ...
        # max default 2 → count becomes 3 → fail
