"""Registry điều phối agent và tool được khai báo trong cấu hình HAgent."""
...
def _build_tool_map() -> dict[str, Any]:
    """
    Tạo ánh xạ tool_name → tool_object.
    Đọc từ automl_tools.ALL_TOOLS; mỗi công cụ đã có thuộc tính .name.
    """
...
def resolve_tools(tool_names: list[str]) -> list[Any]:
    """
    Phân giải danh sách tên công cụ từ YAML thành các đối tượng công cụ.

    Tham số:
        tool_names: Danh sách tên công cụ từ cấu hình YAML.

    Giá trị trả về:
        Danh sách đối tượng công cụ LangChain.
    """
    tmap = get_tool_map()
    tools = []
...
@dataclass
class AgentEntry:
    """Một mục agent đã được phân giải từ cấu hình YAML."""
    ...
    def get_node_function(self) -> Callable:
        """Import động và phân giải hàm node."""
    ...
    def get_tools(self) -> list[Any]:
        """Phân giải tên công cụ thành các đối tượng công cụ."""
...
class AgentRegistry:
    """
    Registry đọc cấu hình YAML và phân giải agent lúc chạy.
    ...
    def get_node_function(self, name: str) -> Callable:
        """Lấy hàm node cho một agent bằng import động."""
    ...
    def get_node_functions(self) -> dict[str, Callable]:
        """Lấy tất cả hàm node theo ánh xạ {name: callable}."""
        self._ensure_loaded()
        return {name: entry.get_node_function() for name, entry in self._agents.items()}

    def get_all_tools(self) -> list[Any]:
        """Lấy toàn bộ công cụ từ mọi agent và loại trùng."""
        self._ensure_loaded()
        seen = set()
        tools = []
...
def reset_registry() -> None:
    """Đặt lại registry khi kiểm thử hoặc khi cấu hình thay đổi."""
