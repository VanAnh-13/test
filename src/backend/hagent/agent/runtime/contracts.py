"""Contract runtime công khai và adapter tương thích graph cũ của HAgent.
...
RuntimeCommand = StartTurn | ResolveApproval | CancelRun
...
TerminalRuntimeEvent = RunCompleted | RunFailed | RunCancelled
...
class RuntimeEventStore(Protocol):
    """Seam đồng bộ tối thiểu cho ledger RAM hoặc Mongo bền vững."""
    ...
    def claim_command(
        self,
        command: ResolveApproval | CancelRun,
        *,
        owner_id: str,
...
class InMemoryRuntimeEventStore:
    """Ledger hữu hạn dành cho compatibility runtime hoặc dev/test rõ ràng."""
    ...
    def __init__(
        ...
        *,
        ...
        self._commands: dict[tuple[str, str], _CommandRecord] = {}
    ...
    def append(self, record: _RunRecord, event: RuntimeEvent) -> None:
        with self._lock:
            ...
            if event.command_id != record.command_id:
                command = self._commands.get((record.owner_id, event.command_id))
                if command is None or command.run_id != record.run_id:
                    raise RuntimeError("Runtime event command was not claimed")
    ...
    def claim_command(
        ...
        *,
        ...
        with self._lock:
            ...
            if command_key in self._command_runs:
                raise RuntimeCommandConflict()
            if record.is_terminal or not record.events or not isinstance(
                record.events[-1], ApprovalRequired
            ):
                raise RuntimeCommandConflict()
            if (
                isinstance(command, ResolveApproval)
                and record.events[-1].approval_id != command.approval_id
            ):
                raise RuntimeCommandConflict()
            claimed = _CommandRecord(
                ...
                command_id=command.command_id,
                fingerprint=fingerprint,
                start_sequence=len(record.events) + 1,
            )
            self._commands[command_key] = claimed
    ...
    def _evict_completed_runs(self) -> None:
        while len(self._runs) >= self._max_runs:
            ...
            for command_key, command in tuple(self._commands.items()):
                if command.run_id == record.run_id:
                    self._commands.pop(command_key, None)
                    self._tombstones.add(
                        _command_tombstone_key(command.owner_id, command.command_id)
                    )
            self._tombstones.add(
...
async def _default_legacy_event_source(
    ...
    from hagent.agent.orchestration.graph import _stream_legacy_graph_events
    ...
    async def dispatch(
        ...
        *,
        ...
        try:
            ...
            if scope.deadline is not None and datetime.now(UTC) >= scope.deadline:
            ...
            while True:
                try:
                    if scope.deadline is None:
                        ...
                        remaining = (
                            scope.deadline - datetime.now(UTC)
                        ).total_seconds()
                        if remaining <= 0:
            ...
            if not terminal_emitted and not record.is_terminal:
                ...
                self._event_store.append(record, failure)
                yield failure
        except Exception:  # noqa: BLE001 - boundary phải đổi lỗi provider thành mã an toàn
            if not terminal_emitted and not record.is_terminal:
                ...
                if legacy_events is not None:
                    ...
                    if close is not None:
                        try:
                            ...
                            raise
                        except Exception as exc:  # noqa: BLE001 - close hook là extension ngoài
                            logger.error(
...
def _command_fingerprint(command: RuntimeCommand) -> str:
    payload = json.dumps(
        asdict(command),
        ensure_ascii=False,
        sort_keys=True,
