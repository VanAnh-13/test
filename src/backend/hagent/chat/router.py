"""
HAgent — Router chat tích hợp HAgent

Các endpoint chat được gắn trực tiếp vào composition root FastAPI.
Sử dụng LangGraph agent runtime — runtime LangGraph duy nhất.
Hỗ trợ cả phản hồi đồng bộ và luồng SSE.
Lưu lịch sử chat vào MongoDB database AutoML.
"""

# FastAPI yêu cầu gọi Depends và File ngay trong chữ ký endpoint.
# ruff: noqa: B008
...
logger = structlog.get_logger("hagent.chat.router")
...
# ─── Hàm hỗ trợ World Model ──────────────────────────────


def _wm_summary(world_model: dict | None) -> dict | None:
    """Rút gọn snapshot cho API/UI để tránh payload quá lớn."""
...
def _apply_request_context(
    ...
    """Chỉ gộp trường request công khai; trạng thái đã lưu trên server được ưu tiên."""
...
async def _load_world_model(db: AsyncDatabase, user_id: str) -> dict | None:
    """Tải snapshot World Model của người dùng qua API kho lưu trữ thống nhất."""
    try:
        ...
        # AsyncDatabase cung cấp hai thuộc tính .client và .name.
    ...
    except Exception as exc:  # noqa: BLE001 - lỗi lưu world model không được chặn request
...
async def _call_agent(
    ...
    *,
    ...
    # Tên model sai (kể cả chuỗi rỗng) → 400, KHÔNG âm thầm dùng mặc định.
...
async def agent_run(
    ...
    """
    Gọi nội bộ agent runtime cho HAgent Bridge.

    Chỉ chạy hệ đa agent LangGraph và không ghi lịch sử hội thoại vì Bridge
    sở hữu kho hội thoại. Endpoint này dành cho dịch vụ Bridge trong Docker.
    """
    ...
    # Lưu lại snapshot world_model của agent nếu kết quả có dữ liệu này.
    if result.get("world_model") and client is not None:
        ...
        except Exception as exc:  # noqa: BLE001 - lỗi lưu response không được chặn request
...
async def agent_run_stream(
    ...
    """Gọi SSE nội bộ không giữ trạng thái; Bridge sở hữu mọi thao tác ghi hội thoại."""
...
async def chat_stream(
    ...
    """
    Endpoint SSE trả dữ liệu theo từng token trong thời gian thực.

    Mỗi frame chứa tên sự kiện, mã tăng dần và trường dữ liệu JSON.
    Tên sự kiện tuân theo hợp đồng stream có kiểu của agent.
    Stream kết thúc bằng đúng một sự kiện done hoặc error và không có frame đánh dấu riêng.
    """
    user_id = current_user["_id"]
    conversation_id = req.conversation_id or uuid.uuid4().hex
    ...
    async def _stream_wrapper():
        """Bọc stream SSE và lưu response cuối cùng vào cơ sở dữ liệu."""
        full_response = ""
        async for chunk in sse_stream(
            ...
            yield chunk
            # Thu thập phản hồi để lưu vào cơ sở dữ liệu.
...
async def health_check():
    ...
    except Exception as exc:  # noqa: BLE001 - lỗi probe phụ không được làm hỏng kiểm tra trạng thái
        logger.debug("HAutoML health probe failed: %s", exc)

    # Liệt kê các model LLM từ cấu hình.
