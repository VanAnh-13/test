...
class SearchStrategyFactory:
    ...
    @classmethod
    def create_strategy(
        cls, strategy_name: str, config: dict[str, Any] | None = None
    ) -> SearchStrategy:
        """
        ...
        if strategy_name.startswith("grid"):
            ...
            strategy_name.startswith("bayesian")
            or strategy_name.startswith("bayes")
            or strategy_name.startswith("skopt")
        ):
    ...
    def is_strategy_available(cls, strategy_name: str) -> bool:
        ...
        strategy_name = strategy_name.lower().strip()
        return (
            strategy_name.startswith("grid")
            or strategy_name.startswith("genetic")
            or strategy_name.startswith("ga")
            or strategy_name.startswith("bayesian")
            or strategy_name.startswith("bayes")
            or strategy_name.startswith("skopt")
            or strategy_name.startswith("random")
            or strategy_name.startswith("rand")
            or strategy_name.startswith("successive")
            or strategy_name.startswith("halving")
            or strategy_name in ("sh", "asha")
        )
